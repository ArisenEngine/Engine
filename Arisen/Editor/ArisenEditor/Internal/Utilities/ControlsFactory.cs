...
using DynamicData;
using Attributes_MenuItem = ArisenEditor.Attributes.MenuItem;
using MenuItem = ArisenEditor.Attributes.MenuItem;
...
internal static class ControlsFactory
{
    ...
    private static void ParseItems(string[] internalMenus, MenuType menuType, out Dictionary<string, MenuItemNode> itemNodes)
    {
        ...
        foreach (var type in types)
        {
            ...
            {
                Attributes_MenuItem attribute = (Attributes_MenuItem) Attribute.GetCustomAttribute(methodInfo, typeof(Attributes_MenuItem));
                if (attribute != null)
                {
                    var menuHierarchies = attribute.menuItem.Split(Attributes_MenuItem.kMenuItemSeparators);
                    ...
                    for (int i = 1; i < menuHierarchies.Length; ++i)
                    {
                        ...
                        var key =  string.IsNullOrEmpty(parentKey) ? childKey : parentKey + Attributes_MenuItem.kMenuItemSeparators + childKey;
                        parentKey = key;
