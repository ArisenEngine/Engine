...
| `StartupWorld` | `object` | Optional | Package-owned `World` descriptor used as the `startupWorld` cooked root and runtime activation root. `PostInit` validates and stages the world and acquires its persistent-scene residency. Ready residency may publish immediately; otherwise the world and persistent scene publish atomically at a later frame boundary. Cells require a streaming source or explicit pin. |
...
3. **Startup Content Selection**: The root composition package prefers `StartupWorld` when present. During `PostInit`, `IRuntimeWorldStreamingService` validates and stages the descriptor and acquires persistent-scene residency. Ready residency may activate immediately; a waiting request returns deferred and frame-boundary processing publishes the world and persistent scene atomically when residency becomes ready. Production cooking closes/deploys the persistent and cell graph under the `startupWorld` root, and streamable cells activate asynchronously after a camera source or explicit pin selects them. Without `StartupWorld`, `StartupScene` remains the synchronous compatibility bootstrap through `IRuntimeSceneService`.
