...
- `DirectionalShadowTarget` owns the first sampled shadow depth image: a reusable `FORMAT_D32_SFLOAT` 2048x2048 image/view/sampler with bindless image and sampler descriptors, expected-layout tracking, and deferred-safe disposal.
...
- `GenericRenderPipeline` declares a transient `DirectionalShadowMap` resource for ordering/diagnostics, records shadow depth before sky/static lighting, and plots shadow map size/enabled state plus pass draw count in Tracy.
...
- `RenderQueuePolicy` now classifies prepared material draws into opaque (`2000`), alpha-test (`2450` via the `ALPHA_TEST` shader keyword), and transparent (`3000` via blend-enabled render state) queues. `StaticMeshPass` sorts its reusable draw-index list by queue, pipeline batch, material, mesh buffers, submesh offset, and source draw index during setup, then records the already-prepared spans without changing the worker-thread hot path. Transparent draws have a deterministic late queue but still use the current static-mesh depth policy until a dedicated transparent pass/depth-write policy is implemented. Tracy plots opaque, alpha-test, and transparent draw counts.
...
- RHI viewport coordinates are top-left with positive width/height. The Vulkan executor implements that contract with a negative native viewport height and compensates front-face winding during pipeline creation; cameras, shaders, and generic render passes remain backend-agnostic.
...
The scene now reads as a small showcase rather than an engine primitive test. This roadmap is complete; the next roadmap should focus on production sample selection and polish.
