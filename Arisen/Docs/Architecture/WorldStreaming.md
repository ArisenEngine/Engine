...
- the interactive camera owners are the composition callers that publish that source. `PackageGame.RuntimeCameraNavigationSubsystem` publishes the runtime free-fly camera pose in interactive non-editor runs and clears it on shutdown. In the Editor, `EditorSceneViewCameraController` publishes the scene-view camera pose only after the user drives the camera or explicitly focuses a cell, so automated sessions and the untouched startup state keep the deterministic pin/preview activation model that the Editor viewport smoke asserts. Because `SetStreamingSource` also requests the origin policy's primary source, a rebased world follows whichever camera currently owns the source.
...
The canonical `LanternWorld` fixture keeps its one gameplay camera, global dusk key light, environment, and non-streamed valley outcrop under the persistent scene. Its cell scenes contain local mesh content authored relative to each cell origin and no competing camera, global light, or environment components: the centre cell owns the 4x4 showcase terrain plus its vegetation clusters, and the west cell owns a boulder outcrop placed inside its own bounds. A cell scene must still contain at least one entity, because `SceneAssetLoader` rejects a scene without entities. Reusing a complete standalone showcase scene as a cell payload is invalid fixture composition: it duplicates global ownership and makes edits appear ineffective because the current render pipeline has no camera-role selector.
...
The canonical scenario follows observable transitions rather than sleeps. It captures `before`, `during`, `unloaded`, and `after` ECS checkpoints; observes `Queued`, `Active`, `Cancelled`, `Unloaded`, and `Failed`; completes four load/unload soak cycles; and verifies active-cell sets, entity/component parity, origin stability, and hard streaming/residency limits every frame. Peak accounting includes allocated ECS slots, cooked handles, in-flight and decoded staging bytes, resident/prepared resources, estimated prepared GPU bytes, prepared descriptors, and pending deferred disposals. After normal package shutdown it requires zero active world cells, scene instances, worker tasks, cooked handles, and residency entries. Every active-cell expectation is derived from the loaded world descriptor and the budgets the scenario configured, not from a hand-written cell list: the desired set is the declared cells inside `LoadRadius` plus already-active cells retained inside `LoadRadius + UnloadHysteresis`, admitted in distance, layer-priority, then stable-id order while each candidate's dependency closure fits `MaxActiveCells`, and a cell whose staging reservation exceeds `MaxDecodedStagingBytes` is expected to fail instead of activating. The cancellation pose is searched across the horizontal load neighbourhood rather than only along X, because a planar cell grid has neighbours on both axes. The same fixture therefore validates the single-lane canonical world and a dense planar open-world grid; `RuntimeWorldStreamingTests.BoundedSmokeScenario_CompletesOnADensePlanarCellGrid` runs the whole scenario against a 2x2 adjacent-cell world and requires a multi-cell active set plus the admission failure.

When visual summaries are enabled, the scenario produces seven independent schema-2 color/depth artifacts: `before`, `during`, `shadow-near`, `shadow-mid`, `shadow-far`, `shadow-far-stable`, and `after`. Captures are scheduled for the following native frame so each observes a completed render/resource-preparation boundary. After the four streaming soak cycles, the persistent validation camera follows a deterministic shadow path: its authored transform is used for `shadow-near`, it retreats 20 world units opposite its forward direction for `shadow-mid`, and it retreats 64 units for `shadow-far`. `shadow-far-stable` captures the immediately following frame without moving the camera, then the authored transform is restored before `after`. Camera world position is converted through `IWorldOriginService`, so this path remains valid after origin rebasing and does not change streaming-source ownership. The scenario pins the kernel animation clock (`Time.Pin`/`Time.Unpin`) for that stationary pair, because the pair is only meaningful when no time-driven input may advance between the two frames, and releases it before `after`; the pin is owned by the scenario, and `RuntimeWorldStreamingTests` asserts exactly one pinned frame, byte-identical stationary captures, and a released clock after shutdown.
...
pixel hashes. Because wind is the renderer's only wall-clock-driven input and the three runs are
separate processes, every comparison run declares `ARISEN_VEGETATION_RENDER_VALIDATION_MODE` and pins
the wind clock to the shared `ComparableWindTimeSeconds` phase before the vegetation feature is
prepared; the byte-identical depth requirement is defined against that pinned pose rather than against
a live gust phase. The relocated cooked-only Production audit runs the same three modes and retains each
comparison summary plus named `during` artifact outside its temporary player root.

`com.arisen.terrain` also provides the independent `terrain-streaming` scenario. It follows deterministic near, boundary, mirror, far, post-rebase, and returned-to-start camera checkpoints, performs exactly one origin rebase, then completes four observable load/reload/unload cycles without sleeps. Fourteen ordered checkpoints verify sixteen stable tile identities, generation advancement after every reload, active ECS/query parity, normalized height/normal/weight queries, conservative world bounds, bounded LOD/patch histograms, zero seam violations, and bounded ECS/cooked/residency/prepared/deferred-disposal state. The fixture does not assume a single owner cell: it derives the owner-cell set of the resident root from the tiles' residency owners, pins, reloads, unpins, and drains every cell in that set, and requires each tile to be held by one of its owner cells under that cell's current generation after every reload. The set is published as `terrainCellIds` (terrain artifact schema 3, alongside the six schema-2 color/depth captures) together with the per-tile `ownerCellIds` that were observed. The final post-shutdown inspection requires every owner cell to be undesired, unpinned, and unloaded or cancelled - a cell the streaming service no longer tracks is published as `tracked: false` and fails the gate instead of counting as drained - with zero visible/runtime/diagnostic/residency resources, pending disposals, and tasks.

Six schema-2 color/depth captures are retained for `near`, `boundary-mixed-lod`, `mirror-cascade`, `far-cascade`, `post-rebase`, and `returned-start`. `validate_terrain_streaming_summary.ps1` requires upright nonblank color and finite written depth, distinct near/boundary/far views, an unchanged tile set, the declared owner-cell set to be exactly the set of cells that own the checkpoint tiles, selected patch plan and world/query state across the rebase, and bounded numeric similarity for the two frame pairs that replay a camera. Equivalent frames use bounded luminance/depth-grid and written-coverage tolerances rather than byte-identical hashes because origin-relative floating-point reconstruction can differ slightly while remaining visually equivalent. The scenario pins the kernel animation clock (`Time.Pin`/`Time.Unpin`) while it replays a captured pose, so neither comparison can be separated by a wind gust, and releases it on every success, failure, and early-unload path. The rebase is isolated instead of folded into a camera path: the fixture moves only the streaming source across the rebase threshold, parks the camera on the far pose captured one frame earlier, and captures that same pose again, so the origin is the only variable left between the compared frames and the validator can require an identical per-tile patch/LOD plan plus LOD histogram for them. The `near`/`returned-start` pair is bounded rather than exact for a stated reason: the far excursion legitimately leaves the hysteresis-locked LOD distribution behind, while the parked pair proves the origin itself changes nothing in the plan. What the origin still costs is small and bounded: on Production the parked frame's terrain cells move by at most 2.3e-5 of mean luminance, average depth by 1.2e-6, the spatial depth grid by 4.2e-6, and written-depth coverage by 2 pixels, while the sky cells - which no origin-relative geometry covers - stay bit-invariant. The tolerances (1e-4 luminance, 2.5e-4 luminance grid, 2e-5 average depth, 3e-5 depth grid, 16 written pixels) remain far below a single intensity step and below what a one-pixel terrain shift would produce.

The coverage contract has two halves, because one frustum cannot contain a root that is wider than it is. Per view, the validator rejects a checkpoint whose own plan view sees a tile that selected no patch; across the flight, it requires the captured views between them to cover every canonical tile of the resident root, recomputed from the per-tile plan-view flags the checkpoints publish and matched against the `coveredTileCount` the artifact declares. The per-view half keeps a view that draws nothing for a tile it frames from passing, and the aggregate half keeps a regional root from quietly reducing the gate to whichever part of the world one pose happens to frame. The validator also rejects an inverted frame, so the surface has to stay under a view that still holds sky above the ridge line. Framing inherited from authored scene content can guarantee neither: the canonical root covers a 512 m square, which is wider than the fixture's roughly 73 degree horizontal frustum at the scene camera's 45 degree vertical field of view and 16:9, so a pose anywhere inside the raster has tiles behind and beside the view direction that select no patch at all, and the authored camera height is only above the surface at the authored position, so a derived pose that inherits it walks into the hillside, captures the inside of the terrain through back-face culling, and reads back a frame whose lower part holds no written depth at all. `TerrainStreamingCameraPath` therefore derives the fixture's own poses from the loaded terrain bounds and the runtime surface query: it captures all four corner regions of the root ordered by distance to the authored camera position - the nearest corner is the `near` pose, the next the `boundary-mixed-lod` pose, the one after it the `mirror-cascade` pose, and the farthest the `far-cascade` pose, which is also the parked pose the rebase replays - keeps only the authored bearing, stands every derived pose `EyeClearanceMetres` above the surface reported at its own horizontal position, and aims at the terrain center with the elevation clamped into a shallow downward band. The mirror pose is the boundary pose reflected across the near-to-far diagonal and exists because a corner region is framed only by a view that stands away from it: on a root wider than the far plane the other three corners cannot reach the corner they turned away from, so a three-corner path covers strictly less than the root. The runtime query answers only while the canonical tiles are active, so the discovery path falls back to the terrain's highest point plus the clearance - above every point of the surface - and the fixture rebuilds the path as soon as the complete render snapshot exists. `TerrainStreamingCameraPathTests` runs the production `TerrainLodView.IsFrustumVisible` predicate the planner culls patches with: on the canonical root it pins that every captured pose frames all sixteen tiles and that the captured poses between them cover the root, pins a synthetic eight-by-eight root wider than one frustum as a negative control that neither a single view nor the near/boundary/far corners cover, raycasts the lower frame onto the terrain floor, and keeps the horizon inside the upper frame, so a framing change fails there instead of in the runtime gate; it cooks those tile bounds from the shipped package content rather than pinning literals, so a raster resize fails the test as well.

Development and Production run the dedicated terrain scenario locally, and copied Production runs it again from an isolated cooked-only player. Every run requires a fresh positive terrain submission marker, an empty Vulkan validation log, clean feature/RHI teardown ordering, a completely drained terrain summary, and all six visual artifacts. The relocated run rejects concrete workspace manifest/cache/package/scene/terrain-source access while allowing harmless embedded debug-symbol paths, then rewrites preserved capture paths out of its temporary player root. Aggregate runtime validation schema 9 records per-profile world/terrain results, three terrain runs, the Development host frame pacing run with its artifact path, the Development and relocated Production vegetation comparisons, and their summary artifact paths. Relocated Production schema 6 records terrain and vegetation closure, both streaming checks, clean shutdown, the terrain/world visuals, and all six vegetation comparison summary/`during` artifact paths.

The relocated Production audit validates terrain closure before boot. Starting from the `startupWorld` and `renderPipeline` catalog roots, it requires the canonical cooked-v2 terrain root, all sixteen generated tiles of its four-by-four raster, exactly four sRGB albedo mip chains, four renormalized linear normal mip chains, four linear ORM mip chains, and three terrain shader stages to be resolved and reachable. It rejects stale or unreferenced terrain-tile rows, wrong texture format versions/variants, deployed terrain layer-set authoring assets, and terrain source extensions including `.ariweights`. Tamper and missing-artifact checks target a terrain tile, and `terrainClosureComplete` is published only after the graph audit succeeds.
