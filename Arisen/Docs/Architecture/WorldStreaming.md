...
Because the validator rejects a checkpoint whose tile selects no patch, every captured checkpoint must keep all sixteen canonical tiles inside the frustum and therefore drawn, and because it also rejects an inverted frame, the surface has to stay under a view that still holds sky above the ridge line. Framing inherited from authored scene content cannot guarantee either: the canonical root covers a 512 m square, which is wider than the fixture's roughly 73 degree horizontal frustum at the scene camera's 45 degree vertical field of view and 16:9, so a pose anywhere inside the raster has tiles behind and beside the view direction that select no patch at all, and the authored camera height is only above the surface at the authored position, so a derived pose that inherits it walks into the hillside, captures the inside of the terrain through back-face culling, and reads back a frame whose lower part holds no written depth at all. `TerrainStreamingCameraPath` therefore derives the fixture's own poses from the loaded terrain bounds and the runtime surface query: it takes the root's four corner regions ordered by distance to the authored camera position - the nearest corner is the `near` pose, the next the `boundary-mixed-lod` pose, and the farthest the `far-cascade` pose, which is also the parked pose the rebase replays - keeps only the authored bearing, stands every derived pose `EyeClearanceMetres` above the surface reported at its own horizontal position, and aims at the terrain center with the elevation clamped into a shallow downward band. The runtime query answers only while the canonical tiles are active, so the discovery path falls back to the terrain's highest point plus the clearance - above every point of the surface - and the fixture rebuilds the path as soon as the complete render snapshot exists. `TerrainStreamingCameraPathTests` culls the cooked canonical tiles with the production frustum test for every captured pose, raycasts the lower frame onto the terrain floor, and pins the horizon inside the upper frame, so a framing change fails there instead of in the runtime gate; it cooks those tile bounds from the shipped package content rather than pinning literals, so a raster resize fails the test as well.
