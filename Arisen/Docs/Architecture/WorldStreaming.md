...
Five schema-2 color/depth captures are retained for `near`, `boundary-mixed-lod`, `far-cascade`, `post-rebase`, and `returned-start`. `validate_terrain_streaming_summary.ps1` requires upright nonblank color and finite written depth, distinct near/boundary/far views, stable world/query state across the rebase, and tight numeric similarity when returning to an equivalent camera/origin state. Equivalent frames use bounded luminance/depth-grid and written-coverage tolerances rather than byte-identical hashes because origin-relative floating-point reconstruction can differ slightly while remaining visually equivalent. The scenario also pins the kernel animation clock (`Time.Pin`/`Time.Unpin`) after restoring the authored camera, so the `boundary-mixed-lod`/`post-rebase` comparison cannot be separated by a wind gust, and releases it on every success, failure, and early-unload path. Vegetation renders in the view frame (see `Rendering.md`), which makes that comparison essentially exact rather than merely close: the Development gate measures an average-luminance delta of 2.13e-8, a spatial luminance grid delta of 1.96e-7, an average depth delta of 5.91e-11, a spatial depth grid delta of 4.88e-10, and identical written-depth coverage.

Because the validator rejects a checkpoint whose tile selects no patch, every captured checkpoint must keep all four canonical tiles inside the frustum and therefore drawn, and because it also rejects an inverted frame, the surface has to stay under a view that still holds sky above the ridge line. Framing inherited from authored scene content cannot guarantee either: the frustum follows the rotation, so a showcase framing change can rotate a tile out of view at the boundary position, and the authored camera height is only above the surface at the authored position, so a derived pose that inherits it walks into the hillside, captures the inside of the terrain through back-face culling, and reads back a frame whose lower part holds no written depth at all. `TerrainStreamingCameraPath` therefore derives the fixture's own poses from the loaded terrain bounds and the runtime surface query: `near`, `returned-start`, and the rebase translation keep replaying the authored showcase position, while every derived pose stands `EyeClearanceMetres` above the surface reported at its own horizontal position, insets the boundary pose into the eastern tile pair along the shared tile boundary, and aims at the terrain center with the elevation clamped into a shallow downward band. The runtime query answers only while the canonical tiles are active, so the discovery path falls back to the terrain's highest point plus the clearance - above every point of the surface - and the fixture rebuilds the path as soon as the complete render snapshot exists. `TerrainStreamingCameraPathTests` culls the canonical tiles with the production frustum test for every checkpoint pose, including the authored one, raycasts the lower frame onto the terrain floor, and pins the horizon inside the upper frame, so a framing change fails there instead of in the runtime gate.
