...
- A logical cluster GUID is a generated biome child keyed by world cell, terrain package/GUID, species package/GUID, and entry identity. The current planner emits one non-empty page whose GUID is a cluster child keyed by page index and a SHA-256 over package-owned scatter identity, page origin, seed data, and the same canonical 40-byte instance records written by the page codec. Changed canonical content therefore selects a new immutable page GUID while an unchanged bake reuses the existing page artifact.
