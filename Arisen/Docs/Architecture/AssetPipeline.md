...
`IAssetDatabase.PublishCookedArtifactSet` is the set-level boundary for generated closures.
It stages every non-empty payload, preserves byte-identical records, backs up replaced or
removed cache files, atomically replaces one candidate manifest/snapshot, and only then
deletes the private backup. Any staging, file, or manifest failure restores the prior
registry and cache paths; callers provide exact removal identities rather than allowing
the database to scan unrelated cache content.
...
- `TerrainImportPlanner` is the package-neutral terrain creation boundary. Given one indexed layer set, a strict PGM source, an optional authored `.ariweights` `Rgba8Hex` source whose raster must match the height raster exactly, package `Assets` root, output folder/name, double-precision world bounds, shared-edge tile resolution, and signed origin, it derives sample spacing, canonical generated paths, stable child GUIDs, and optional active-world cell ownership/intersections. The authored weight raster is used verbatim; without one the planner keeps the existing owned raster when its dimensions still match and otherwise writes a deterministic channel-zero default. Planning is read-only: it validates every input and existing owned output, returns diagnostics plus a content/state fingerprint over every observed source byte, and never creates the destination directory or metadata.
...
- `CookedTerrainSurfaceSampler` is the shared decoded-data sampling kernel for setup-time consumers and active runtime terrain queries. It validates a complete root/tile closure, exact root/reference/tile identity, canonical placement, shared height/weight borders, finite vertical and gradient domains, and strictly increasing world-axis boundaries. Signed coordinates and decimal spacing use the same canonical source-sample placement arithmetic as terrain cooking: a positive interior border belongs to the positive tile and the outer maximum remains inclusive. Bilinear height, scaled finite-normal reconstruction, and normalized layer weights allocate nothing per sample.

The vegetation authoring, cooked-data, and first scatter-bake slices are owned by `com.arisen.vegetation`:
...
- The page format also writes a non-required acceleration extension with one canonical range per species: first/last instance index, exact species AABB, instance count, and maximum conservative radius. Readers validate every range against the decoded instance payload; legacy pages without the extension remain readable and rebuild the same range data in memory.
...
- Cluster roots add paired non-required acceleration extensions: a deterministic median-split hierarchy of conservative page AABBs, and one record per canonical species containing covered page range/count, instance count, exact species AABB, maximum conservative radius, and farthest LOD distance/screen-error thresholds. The fixed-width little-endian records reject noncanonical bounds, missing/duplicate leaves, unreachable nodes, invalid ranges, nonzero padding, and malformed nonnegative LOD values; old roots without these sections remain readable.
...
- `VegetationScatterBaker.Build(IAssetDatabase, VegetationScatterCookRequest)` is the trusted setup-time scatter boundary. It loads the exact cooked biome, selected species, terrain root, and every root-declared tile; each tile must match its catalog owner/type/variant plus the root's exact byte-size and full-file SHA-256 pin before decoding. The descriptor-based overload is internal so publication-ready output cannot bypass those database checks.
- Candidate sequence version 1 derives one SHA-256 base seed from explicit world, biome, terrain-root, species, entry, and authored seed identity, then uses stateless signed one-meter lattice coordinates, ordinal, and stable-key lanes for density, jitter, scale, yaw, and tilt. Iteration order, terrain-tile input order, interleaving, and parallel bake scheduling cannot change output. Candidate work is bounded at `1,048,576` per cell bake.
- Terrain-backed candidates evaluate bilinear height, finite unit normal, slope, altitude, normalized layer weights, density, hard/soft exclusions, and minimum spacing. Placements first narrow to the canonical owner-cell local `float3` and reconstruct double-world position; ownership is recomputed until stable. Stable-key spacing uses sparse spacing-sized buckets populated incrementally, so a candidate searches only already inserted lower-key candidates. A one-spacing halo lets adjacent-cell candidates participate, while only candidates owned by the requested half-open world cell are emitted. Rejection metrics include density, terrain, rules, exclusions, spacing, ownership, and acceptance and must reconcile exactly to the candidate count.
- A logical cluster GUID is a generated biome child keyed by world cell, terrain package/GUID, species package/GUID, and entry identity. The planner emits non-empty pages whose GUIDs are cluster children keyed by page index and a SHA-256 over package-owned scatter identity, page origin, seed data, and the same canonical 40-byte instance records written by the page codec. Changed canonical content therefore selects a new immutable page GUID while an unchanged page reuses the existing page artifact.
- The planner accepts the bounded cluster output, partitions it in stable-key order using the authored `ClusterSize` as the per-page target, and rejects page-count or total-instance overflow before publication. It returns cluster/page descriptors, generated metadata, per-page hashes, and the cluster placement hash without mutating the asset database. The recipe generator stages the complete generated source set, refreshes and validates its ownership, then prepares every page/root payload before publishing through `IAssetDatabase.PublishCookedArtifactSet`.
- A recipe replacement is one source/cooked transaction. The source transaction backs up every prior generated page/cluster file, installs the complete new set, and refreshes the source index. The cooked set publisher preserves byte-identical pages, removes only the exact stale cluster/page identities from the prior generated directory, and atomically switches the candidate manifest/snapshot. A source-index, payload, or manifest failure restores the previous generated files and refreshes their catalog closure.

The scatter slice now defines an explicit valid-empty/no-output plan, bounded multi-page
partitioning, and transactional page-set replacement. A no-output bake carries the
deterministic logical cluster identity and metrics, removes the prior generated closure,
and publishes no cluster or page artifact. The scene/runtime boundary is now present:
...
indexed opaque and cascaded-shadow passes without reopening cooked or source assets. Cooking-side
page/hierarchy acceleration is now present; runtime hierarchical culling/LOD, broad foliage
materials, and backend-specific GPU-driven behavior remain later vegetation work.
