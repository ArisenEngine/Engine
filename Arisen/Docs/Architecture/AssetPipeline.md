...
Arisen Engine is converging on a strict **Cooked Binary Asset Pipeline**. Shader, texture, environment, material, mesh, scene, and world-descriptor formats now have implemented cooked slices. The package-neutral closed-set cooking contract, real package-owned cooker providers, generated Production cook host, relocatable incremental deployment, cooked-default runtime selection, explicit source diagnostics, and copied-output validation are in place.
...
The current `IAssetDatabase` reads a cooked artifact into managed memory and returns a generation-checked `CookedAssetHandle`. Each format validates its own magic, version, counts, offsets, sizes, and semantic constraints before setup consumes the payload.
...
- `com.arisen.generic-renderpipeline`: `RenderPipelineSettings`, including the default material, always-prepared fallback mesh, and code-owned directional-shadow, environment-sky, and tonemap shader-stage variants.
...
1. **Development Cache Registry**: During local cooking, the Asset Pipeline generates `.arisen/Cache/CookedAssets/AssetManifest.json`, which maps `Guid + Variant` to mutable workspace artifact records and may contain local paths/timestamps. It is not deployable runtime metadata. Production cooking creates and deploys the separate validated `runtime-assets.json` above; ordinary Production boot mounts that catalog read-only and does not index workspace source or consume the development cache registry.
2. **Runtime Loading**: When a Guid is requested, the Database checks its GUID index and cooked artifact manifest. If it is not loaded, the current implementation reads the cooked file and hands back a generation-checked `CookedAssetHandle`.
...
- `Texture2DAssetCooker` for source-time `.ppm` parsing or PNG/JPEG image decode, optional complete packed mip-chain generation, cooked binary header/payload emission, cooked manifest registration, and `CookedAssetHandle` loading. Linear variants use channel-space box filtering; sRGB variants filter RGB in linear light and alpha linearly.
...
- `SceneComponentSchemas` is the explicit source/cooked codec registry. Stable numeric type IDs map to canonical names, current versions, typed source readers/writers, and migration steps. A scene declares the component schemas it uses. Unknown or newer required components fail; unknown or newer optional components are removed before typed parsing. Camera v1 to v2 (`FieldOfView` to `VerticalFov`) is the first real migration. Missing migration links fail deterministically, and runtime ECS structs remain data-only.
...
Each `Texture2DRefs` entry may also author binding-local `Sampler` metadata (`MinFilter`, `MagFilter`, `MipmapMode`, `WrapU`, `WrapV`) and `Transform` metadata (`Offset`, `Scale`, `Rotation`, `TexCoord`). The same image asset may therefore be reused by bindings with different sampling intent without making sampler state part of pixel cooking. Omitted sampler metadata resolves to linear min/mag filtering, linear mip interpolation, and repeat wrapping; omitted transform metadata resolves to zero offset, unit scale, zero rotation, and UV set zero. `GenerateMipMaps` remains part of the texture cooked variant rather than sampler identity. `MaterialAssetLoader` validates finite transform data, version 6 `material.runtime` payloads preserve the resolved values, `Texture2DAssetCooker` writes the requested packed chain, `RHITexture2DResource` creates the resolved full-LOD sampler during setup, and `RHIMaterialResource` caches the transform beside the bindless binding for later shader preparation.
