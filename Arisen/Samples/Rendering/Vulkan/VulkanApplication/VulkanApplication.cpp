...
std::vector<const char*> VulkanApplication::InstanceExtensionNames
{
...
std::vector<const char*> VulkanApplication::DeviceExtensionNames
{
...
/// <summary>
/// invoke after vkInstance has been created
/// </summary>
...
void VulkanApplication::CreateGraphicsPipeline()
{
	...
	char* buf = nullptr;
	size_t sz = 0;
	std::string pathvarStr;
	if (_dupenv_s(&buf, &sz, "ARISEN_SOLUTION_ROOT") == 0 && buf != nullptr)
	{
		printf("EnvVarName = %s\n", buf);
		pathvarStr = buf;
		free(buf);
	...
	std::cout << "Solution Root is: " << pathvarStr << std::endl;
	auto vertShaderCode = AppReadFile(pathvarStr +"/x64/Debug/SimpleTriangle.vert.spv");
	auto fragShaderCode = AppReadFile(pathvarStr + "/x64/Debug/SimpleTriangle.frag.spv");
