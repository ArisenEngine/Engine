...
namespace ArisenEngine::Testing
{
    ...
    void RHIRenderingTestBase::InitShaderProgram(const String& shaderName)
    {
        ...
        {
            RHI::RHIShaderProgramDesc desc = { (UInt32)outputVertex.codeSize, outputVertex.codePointer, "Vert", path.c_str(), RHI::SHADER_STAGE_VERTEX_BIT };
        ...
        {
            RHI::RHIShaderProgramDesc desc = { (UInt32)outputFragment.codeSize, outputFragment.codePointer, "Frag", path.c_str(), RHI::SHADER_STAGE_FRAGMENT_BIT };
    ...
    void RHIRenderingTestBase::UploadImage(RHI::RHIImageHandle textureHandle, UInt64 imageSize, void* data, UInt32 texWidth, UInt32 texHeight, RHI::EImageLayout finalLayout)
    {
...

        {
            ...
            cmd->CopyBufferToImage(stagingBuffer, textureHandle, RHI::IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, std::move(regions));
        }
