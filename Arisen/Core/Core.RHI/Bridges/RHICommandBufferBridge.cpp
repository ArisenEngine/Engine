...
RHI_DLL void RHICommandBuffer_BeginRenderPass(RHICommandBuffer* cb, 
    ...
    int subpassContents, uint32_t clearValueCount, const RHIClearValue* pClearValues)
{
...
RHI_DLL void RHICommandBuffer_SetViewport(RHICommandBuffer* cb, float x, float y, float width, float height, float minDepth, float maxDepth)
{
    cb->SetViewport(x, y, width, height, minDepth, maxDepth);
}

RHI_DLL void RHICommandBuffer_SetScissor(RHICommandBuffer* cb, uint32_t offsetX, uint32_t offsetY, uint32_t width, uint32_t height)
{
...
RHI_DLL void RHICommandBuffer_BindIndexBuffer(RHICommandBuffer* cb, RHIBufferHandle buffer, uint64_t offset, int indexType)
{
    cb->BindIndexBuffer(buffer, offset, static_cast<EIndexType>(indexType));
}

RHI_DLL void RHICommandBuffer_Draw(RHICommandBuffer* cb, uint32_t vertexCount, uint32_t instanceCount, uint32_t firstVertex, uint32_t firstInstance, uint32_t firstBinding)
{
    cb->Draw(vertexCount, instanceCount, firstVertex, firstInstance, firstBinding);
}

RHI_DLL void RHICommandBuffer_DrawIndexed(RHICommandBuffer* cb, uint32_t indexCount, uint32_t instanceCount, uint32_t firstIndex, int vertexOffset, uint32_t firstInstance, uint32_t firstBinding)
{
...
RHI_DLL void RHICommandBuffer_PipelineBarrier(RHICommandBuffer* cb, int srcStage, int dstStage, uint32_t dependency,
    ...
    const RHIImageMemoryBarrier* pImageMemoryBarriers, uint32_t imageMemoryBarrierCount,
    const RHIBufferMemoryBarrier* pBufferMemoryBarriers, uint32_t bufferMemoryBarrierCount)
{
...
RHI_DLL void RHICommandBuffer_TransitionImageLayoutExplicit(RHICommandBuffer* cb, RHIImageHandle image, int oldLayout, int targetLayout)
{
    cb->TransitionImageLayout(image, static_cast<EImageLayout>(oldLayout), static_cast<EImageLayout>(targetLayout));
}

RHI_DLL void RHICommandBuffer_BindDescriptorSets(RHICommandBuffer* cb, int bindPoint, uint32_t firstSet, RHIDescriptorPoolHandle poolHandle, uint32_t poolId)
{
    cb->BindDescriptorSets(static_cast<EPipelineBindPoint>(bindPoint), firstSet, poolHandle, poolId);
}

RHI_DLL void RHICommandBuffer_PushConstants(RHICommandBuffer* cb, uint32_t offset, uint32_t size, const void* data, uint32_t stageFlags)
{
    cb->PushConstants(offset, size, data, stageFlags);
}

RHI_DLL void RHICommandBuffer_CopyBuffer(RHICommandBuffer* cb, RHIBufferHandle src, uint64_t srcOffset, RHIBufferHandle dst, uint64_t dstOffset, uint64_t size)
{
