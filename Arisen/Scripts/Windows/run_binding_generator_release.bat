...
echo [1/2] Refreshing Source Interop with BindingGenerator (Release)...
...
echo [RUN] dotnet run --project "%GEN_CSPROJ%" --configuration Release -- --source_code "%ROOT_DIR%\Development" --output "%ROOT_DIR%\Development" >> "%GEN_LOG%"
dotnet run --project "%GEN_CSPROJ%" --configuration Release -- --source_code "%ROOT_DIR%\Development" --output "%ROOT_DIR%\Development" >> "%GEN_LOG%" 2>&1
