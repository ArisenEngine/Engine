...
if /i "%SMOKE_MODE%"=="terrain-streaming" exit /b 0
...
echo [ERROR] Expected one of: boot, scene, hot-reload, world-streaming, terrain-streaming
...
set "CURRENT_TERRAIN_STREAMING_LOG_PATH="
set "CURRENT_TERRAIN_STREAMING_PASSED="
...
if errorlevel 1 (
    ...
    set "FAILED_PROFILE=%CURRENT_PROFILE%"
    set "FAILURE_STAGE=runtime package shutdown"
    set "FAILURE_MESSAGE=Runtime package shutdown validation failed for profile %CURRENT_PROFILE%"
    set "RESULT_PROFILE=%CURRENT_PROFILE%"
    set "RESULT_STATUS=failed"
    set "RESULT_REQUIRES_VULKAN=%PROFILE_REQUIRES_VULKAN%"
    set "RESULT_EXIT_CODE=1"
...
call :validate_runtime_camera_idle
if errorlevel 1 (
    echo [ERROR] Runtime camera-idle validation failed for profile %CURRENT_PROFILE%.
    set "FAILED_PROFILE=%CURRENT_PROFILE%"
    set "FAILURE_STAGE=runtime camera idle"
    set "FAILURE_MESSAGE=Runtime camera-idle validation failed for profile %CURRENT_PROFILE%"
    set "RESULT_PROFILE=%CURRENT_PROFILE%"
    set "RESULT_STATUS=failed"
    set "RESULT_REQUIRES_VULKAN=%PROFILE_REQUIRES_VULKAN%"
    set "RESULT_EXIT_CODE=1"
...
if "!CURRENT_TERRAIN_STREAMING_REQUIRED!"=="1" (
    call :run_terrain_streaming_smoke
    if errorlevel 1 (
        echo [ERROR] Terrain-streaming smoke failed for profile %CURRENT_PROFILE%.
        set "FAILED_PROFILE=%CURRENT_PROFILE%"
        set "FAILURE_STAGE=terrain-streaming smoke"
        set "FAILURE_MESSAGE=Terrain-streaming smoke failed for profile %CURRENT_PROFILE%"
        set "RESULT_PROFILE=%CURRENT_PROFILE%"
        set "RESULT_STATUS=failed"
        set "RESULT_REQUIRES_VULKAN=%PROFILE_REQUIRES_VULKAN%"
        set "RESULT_EXIT_CODE=1"
        set "RESULT_LOG_PATH=!CURRENT_TERRAIN_STREAMING_LOG_PATH!"
        set "RESULT_MESSAGE=!FAILURE_MESSAGE!"
        call :record_result
        exit /b 1
    )
...
:validate_runtime_camera_idle
powershell -NoProfile -ExecutionPolicy Bypass -Command "$resolvedPath = Join-Path $env:BIN_DIR 'manifest.resolved.json'; if (-not (Test-Path -LiteralPath $resolvedPath -PathType Leaf)) { Write-Host ('[ERROR] Runtime resolved manifest is missing for camera-idle validation: {0}' -f $resolvedPath); exit 1 }; try { $resolved = Get-Content -LiteralPath $resolvedPath -Raw | ConvertFrom-Json } catch { Write-Host ('[ERROR] Runtime resolved manifest is invalid for camera-idle validation: {0}' -f $_.Exception.Message); exit 1 }; $packageIds = @($resolved.ResolvedPackages | ForEach-Object { [string]$_.Id }); if ($packageIds -notcontains 'com.arisen.packagegame') { exit 0 }; $logPath = $env:CURRENT_PLAYER_LOG_PATH; if ([string]::IsNullOrWhiteSpace($logPath) -or -not (Test-Path -LiteralPath $logPath -PathType Leaf)) { Write-Host ('[ERROR] Launch-owned player log is missing for camera-idle validation: {0}' -f $logPath); exit 1 }; $text = Get-Content -LiteralPath $logPath -Raw; if ($env:CURRENT_PROFILE -ieq 'Editor') { $editorMarker = '[RuntimeCamera] Editor build detected;'; if ($text.IndexOf($editorMarker, [StringComparison]::Ordinal) -lt 0) { Write-Host ('[ERROR] Editor runtime log does not confirm that the editor host owns camera navigation: {0}' -f $logPath); exit 1 }; Write-Host ('[Arisen] Runtime camera idle passed: editor host owns navigation ({0})' -f $logPath); exit 0 }; $smokeMarker = '[RuntimeCamera] Bounded smoke run '''; if ($text.IndexOf($smokeMarker, [StringComparison]::Ordinal) -lt 0) { Write-Host ('[ERROR] Bounded smoke run {0} did not confirm that interactive camera navigation stays disabled: {1}' -f $env:CURRENT_PROFILE, $logPath); exit 1 }; Write-Host ('[Arisen] Runtime camera idle passed: bounded smoke run owns world activation ({0})' -f $logPath)"
exit /b %ERRORLEVEL%
...
for %%N in (before during shadow-near shadow-mid shadow-far shadow-far-stable after) do del /q "%LOG_DIR%\world-streaming-visual-%CURRENT_PROFILE%-latest.%%N.json" >nul 2>nul
...
if errorlevel 1 exit /b 1
powershell -NoProfile -ExecutionPolicy Bypass -File "%SCRIPT_ROOT%validate_cascaded_shadow_visuals.ps1" -SummaryPath "!CURRENT_WORLD_STREAMING_SUMMARY_PATH!" -LogDirectory "%BIN_DIR%\logs" -ExpectedCascadeCount 4 -StartedUtc "!CURRENT_WORLD_STREAMING_STARTED_UTC!"
if errorlevel 1 exit /b 1
powershell -NoProfile -ExecutionPolicy Bypass -File "%SCRIPT_ROOT%validate_outdoor_atmosphere_visuals.ps1" -SummaryPath "!CURRENT_WORLD_STREAMING_SUMMARY_PATH!" -LogDirectory "%BIN_DIR%\logs" -ExpectedSkyMode Panorama -StartedUtc "!CURRENT_WORLD_STREAMING_STARTED_UTC!"
if errorlevel 1 exit /b 1
...
set "CURRENT_TERRAIN_STREAMING_REQUESTED=1"
set "CURRENT_TERRAIN_STREAMING_SUMMARY_PATH=%LOG_DIR%\terrain-streaming-summary-%CURRENT_PROFILE%-latest.json"
set "CURRENT_TERRAIN_STREAMING_VISUAL_BASE_PATH=%LOG_DIR%\terrain-streaming-visual-%CURRENT_PROFILE%-latest.json"
set "CURRENT_TERRAIN_STREAMING_LOG_PATH=%LOG_DIR%\terrain-streaming-%CURRENT_PROFILE%-%CONFIG%-%RUN_TIMESTAMP%.log"
set "CURRENT_TERRAIN_STREAMING_EXIT_CODE="
...
if /i "%CURRENT_PROFILE%"=="Production" (
    ...
    "%EXE_PATH%" --workspace "%WORKSPACE_DIR%" --profile "%CURRENT_PROFILE%" --smoke-mode terrain-streaming --frames 1 --smoke-summary-output "!CURRENT_TERRAIN_STREAMING_SUMMARY_PATH!" --visual-summary --visual-summary-output "!CURRENT_TERRAIN_STREAMING_VISUAL_BASE_PATH!" > "!CURRENT_TERRAIN_STREAMING_LOG_PATH!" 2>&1
)
set "CURRENT_TERRAIN_STREAMING_EXIT_CODE=!ERRORLEVEL!"
popd >nul
...
set "CURRENT_TERRAIN_STREAMING_PASSED=1"
exit /b 0
...
powershell -NoProfile -ExecutionPolicy Bypass -File "%SCRIPT_ROOT%validate_vegetation_submission_log.ps1" -LogPath "%CURRENT_PLAYER_LOG_PATH%" -CaseName "%CURRENT_PROFILE%" -RequiredDistinctSurfaceCount %CURRENT_VEGETATION_REQUIRED_SURFACE_COUNT% -ResolvedManifestPath "%BIN_DIR%\manifest.resolved.json" -RequiredPackageId "com.arisen.vegetation.generic-renderpipeline"
exit /b %ERRORLEVEL%
...
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0validate_editor_viewport_summary.ps1" -ArtifactPath "%CURRENT_EDITOR_VIEWPORT_SMOKE_PATH%" -ExpectedProfile "%CURRENT_PROFILE%"
set "CURRENT_EDITOR_VIEWPORT_SMOKE_EXIT_CODE=%ERRORLEVEL%"
exit /b %CURRENT_EDITOR_VIEWPORT_SMOKE_EXIT_CODE%
