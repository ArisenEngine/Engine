...
REM ============================================================================
REM ArisenEngine - Build EditorTest (Release Only)
REM ----------------------------------------------------------------------------
REM Usage:
REM   build_editor_test_release.bat [--no-pause]
REM ============================================================================
...
set VS_BUILD_DIR=%ROOT_DIR%\Projects\VisualStudio\EditorTest
if not exist "%VS_BUILD_DIR%" mkdir "%VS_BUILD_DIR%"
set "LOG_FILE=%VS_BUILD_DIR%\build.log"
echo === EditorTest Build Log (Release Only) === > "%LOG_FILE%"
...
call :run cmake -S "%ROOT_DIR%" -B "%VS_BUILD_DIR%" -DTARGET="EditorTest" -DPLATFORM="Windows" -G "Visual Studio 17 2022" -A x64
if errorlevel 1 goto :fail

REM 2. Add C# Projects
set /a STEP_INDEX+=1
echo [%STEP_INDEX%/%STEP_TOTAL%] Adding .csproj to solution
call :run call "%SCRIPT_DIR%\dotnet_add_csproj_editor_test.bat" "%VS_BUILD_DIR%\EditorTest.sln" "%VS_BUILD_DIR%\Outputs"
if errorlevel 1 goto :fail

REM 3. Group Solution
set /a STEP_INDEX+=1
echo [%STEP_INDEX%/%STEP_TOTAL%] Grouping solution folders
call :run python "%SCRIPT_DIR%\..\group_sln_cs.py" "%VS_BUILD_DIR%\EditorTest.sln"
if errorlevel 1 goto :fail
