...
REM ==== 1. 创建构建目录（如果不存在）====
set VS_BUILD_DIR=!ROOT_DIR!\Projects\VisualStudio\NativeEngineTest
if not exist "!VS_BUILD_DIR!" (
    mkdir "!VS_BUILD_DIR!"
)

set "LOG_FILE=!VS_BUILD_DIR!\build.log"
echo === NativeEngineTest Build Log === > "!LOG_FILE!"
...
call :step "Configuring CMake (multi-config solution)" cmake -S "!ROOT_DIR!" -B "!VS_BUILD_DIR!" -DTARGET="NativeEngineTest" -DPLATFORM="Windows" -G "Visual Studio 17 2022" -A x64
