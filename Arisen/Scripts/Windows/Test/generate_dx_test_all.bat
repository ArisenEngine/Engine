...
REM ============================================================================
REM ArisenEngine - Generate and Build DXTest (VS multi-config solution)
REM ----------------------------------------------------------------------------
REM Usage:
REM   generate_dx_test_all.bat
REM
REM This script:
...
REM ==== 1. 创建构建目录（如果不存在）====
set VS_BUILD_DIR=!ROOT_DIR!\Projects\Visual Studio\!TARGET!
if not exist "!VS_BUILD_DIR!" (
    mkdir "!VS_BUILD_DIR!"
)

set "LOG_FILE=!VS_BUILD_DIR!\build.log"
echo === DXTest Build Log === > "!LOG_FILE!"
...
call :step "Configuring CMake (multi-config solution)" cmake -S "!ROOT_DIR!" -B "!VS_BUILD_DIR!" -DTARGET="!TARGET!" -DPLATFORM="!PLATFORM!" -G "Visual Studio 17 2022" -A x64
